defmodule Corals.Resolver do
  ...
  defp many rules, opts, user_opts, nil do
    reduce_while rules, opts, &(cont_many? merge &2, resolve_raw(&2, &1, user_opts, &2))
  end
  defp many rules, opts, user_opts, globals do
    reduce_while rules, opts, &(cont_many? merge &2, resolve_raw(&2, &1, user_opts, globals))
  end
  ...
  defp list(rules, opts, user_opts, globals) when is_list(user_opts) and (length(opts) == length(user_opts)) do
    zip(opts, user_opts) |> map(&(resolve_raw elem(&1,0), rules, elem(&1,1), globals))
  end
  defp list rules, opts, _, globals do
    opts |> map(&(resolve_raw &1, rules, %{}, globals))
  end
